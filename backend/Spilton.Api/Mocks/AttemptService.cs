...
{
 protected override async Task ExecuteAsync(CancellationToken stoppingToken){while(!stoppingToken.IsCancellationRequested){try{using var scope=scopes.CreateScope();var db=scope.ServiceProvider.GetRequiredService<AppDbContext>();var service=scope.ServiceProvider.GetRequiredService<AttemptService>();var expired=await db.MockAttempts.AsNoTracking().Where(a=>a.Status=="ACTIVE"&&a.ExpiresAt<=service.Now).OrderBy(a=>a.ExpiresAt).Take(100).Select(a=>new{a.UserId,a.Id}).ToListAsync(stoppingToken);foreach(var a in expired)await service.Expire(a.UserId,a.Id,stoppingToken);}catch(OperationCanceledException)when(stoppingToken.IsCancellationRequested){break;}catch(Exception e){logger.LogWarning("Attempt expiry will retry after {Type}",e.GetType().Name);}try{await Task.Delay(5000,stoppingToken);}catch(OperationCanceledException){break;}}}
}
