...
public class MockTests:IClassFixture<MockFactory>
{
 ...
 Assert.All(result.GetProperty("questions").EnumerateArray(),q=>{var review=q.GetProperty("review");Assert.Equal(JsonValueKind.String,review.GetProperty("correctOptionId").ValueKind);Assert.False(string.IsNullOrWhiteSpace(review.GetProperty("explanation").GetString()));});
