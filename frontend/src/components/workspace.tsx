...
export function Workspace({ user: initialUser }: { user: User }) {
  ...
  async function sendVerification(){setProfileBusy(true);setProfileMessage('');try{await apiRequest('/auth/request-email-verification',{});setProfileMessage('Verification code sent. Check your email.');}catch(e){setProfileMessage(e instanceof Error?e.message:'Unable to send verification email.');}finally{setProfileBusy(false);}}
  async function verifyEmail(){setProfileBusy(true);setProfileMessage('');try{const updated=await apiRequest<User>('/auth/verify-email',{code:verificationCode});setUser(updated);setVerificationCode('');setProfileMessage('Email verified.');}catch(e){setProfileMessage(e instanceof Error?e.message:'Unable to verify email.');}finally{setProfileBusy(false);}}
  ...
  return <div className="sp-app">
    ...
    {settings && <div className="sp-modal-backdrop"><section role="dialog" aria-modal="true" aria-labelledby="model-settings-title" className="sp-modal"><button className="sp-modal-close" aria-label="Close settings" onClick={() => setSettings(false)}><X size={19}/></button><span className="sp-mini-label">SPILTON SETTINGS</span><h2 id="model-settings-title">Your profile</h2><form onSubmit={saveProfile}><label>Display name<input value={profileName} onChange={e=>setProfileName(e.target.value)} required maxLength={100}/></label><label>Email<input value={user.email} disabled/></label>{!user.emailVerifiedAt&&<><p className="sp-settings-note">Email not verified.</p><button type="button" className="sp-settings-done" disabled={profileBusy} onClick={sendVerification}>Send verification code</button><label>Six-digit code<input value={verificationCode} onChange={e=>setVerificationCode(e.target.value.replace(/\D/g,'').slice(0,6))} inputMode="numeric" pattern="[0-9]{6}" maxLength={6}/></label><button type="button" className="sp-settings-done" disabled={profileBusy||verificationCode.length!==6} onClick={verifyEmail}>Verify email</button></>}{user.emailVerifiedAt&&<p className="sp-settings-note">Email verified.</p>}{profileMessage&&<p role="status" className="sp-settings-note">{profileMessage}</p>}<button className="sp-settings-done" disabled={profileBusy||!profileName.trim()}>{profileBusy?'Saving…':'Save profile'}</button></form><h2>Your model connection</h2><p>Spilton Auto uses the configured default model. Only server-configured models appear in the selector.</p>{models.length ? models.map(m => <div className="sp-model-card" key={m.id}><strong>{m.label}</strong><small>{m.isDevelopment ? 'Scripted demo · no real AI connection' : 'Configured transport · access depends on provider credentials'}</small></div>) : <p>No model is configured.</p>}<p className="sp-settings-note">API keys stay on the server and are never entered here.</p><button type="button" className="sp-settings-done" onClick={() => setSettings(false)}>Done</button></section></div>}
  </div>;
}
