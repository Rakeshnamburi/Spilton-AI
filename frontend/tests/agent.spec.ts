...
test('bounded agent calculates, streams progress, persists and uses configured web research',async({page})=>{
 ...
 await page.goto('/register');await page.getByLabel('Full name').fill('Agent Explorer');await page.getByLabel('Email').fill(email);await page.getByLabel('Password',{exact:true}).fill(password);await page.getByRole('button',{name:'Create account',exact:true}).click();await expect(page).toHaveURL(/verify-email/); await page.getByRole('link',{name:'Continue and verify later'}).click(); await expect(page).toHaveURL(/\/chat$/);
